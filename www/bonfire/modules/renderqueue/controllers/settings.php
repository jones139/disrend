...
class settings extends Admin_Controller {
	...
	public function __construct() 
	{
		...
		$this->auth->restrict('RenderQueue.Settings.View');
	...
	*/
	...
	{
		$this->auth->restrict('RenderQueue.Settings.Create');
		...
		if ($this->input->post('submit'))
		{
			if ($insert_id = $this->save_renderqueue())
			{
				...
				Template::redirect(SITE_AREA .'/settings/renderqueue');
			}
			else 
			{
	...
	*/
	...
	{
		$this->auth->restrict('RenderQueue.Settings.Edit');
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/settings/renderqueue');
		}
	...
	*/
	...
	{	
		$this->auth->restrict('RenderQueue.Settings.Delete');
...
$this->form_validation->set_rules('renderqueue_status','Status','max_length[4]');			
$this->form_validation->set_rules('renderqueue_rendererId','Renderer','max_length[4]');			
$this->form_validation->set_rules('renderqueue_statusDate','Status Date','');			
