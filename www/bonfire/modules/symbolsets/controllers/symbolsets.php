...
class symbolsets extends Front_Controller {
	...
	public function __construct() 
	{
		...
		Assets::add_js($this->load->view('symbolsets/js', null, true), 'inline');
	...
	*/
	...
	{
		$this->auth->restrict('SymbolSets.Symbolsets.Create');
		...
		if ($this->input->post('submit'))
		{
			if ($insert_id = $this->save_symbolsets())
			{
				...
				Template::redirect(SITE_AREA .'/symbolsets/symbolsets');
			}
			else 
			{
	...
	*/
	...
	{
		$this->auth->restrict('SymbolSets.Symbolsets.Edit');
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/symbolsets/symbolsets');
		}
	...
	*/
	...
	{	
		$this->auth->restrict('SymbolSets.Symbolsets.Delete');
...
$this->form_validation->set_rules('symbolsets_Name','Name','max_length[50]');			
$this->form_validation->set_rules('symbolsets_symbolSetFile','SymbolSetFile','');			
$this->form_validation->set_rules('symbolsets_thumbnail','T','');
